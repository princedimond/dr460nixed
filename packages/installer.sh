...
confirm_choices() {
    local ANSWER
    read -rp "Are you sure you want to continue? $1 [y/n] " ANSWER
...
5) ZFS encrypted"
    ...
    while [ -z "${DISKO_MODULE+x}" ]; do
        case "${_LAYOUT}" in
            1) DISKO_MODULE=btrfs-subvolumes ;;
            2) DISKO_MODULE=luks-btrfs-subvolumes ;;
            3) DISKO_MODULE=simple-efi ;;
            4) DISKO_MODULE=zfs-encrypted ;;
            5) DISKO_MODULE=zfs ;;
    ...
    while [ -z "${_VALID_DISK+x}" ]; do
        read -rp "Specify the disk you want to use, e.g., \"nvme0n1\": " DISK
